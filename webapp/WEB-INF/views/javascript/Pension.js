    ...
    function addPensionAmount(inputId, buttonId, listId) {
        $("#" + buttonId).click(function () {
            ...
            if (!isNaN(pensionAmount) && pensionAmount > 0) {
                ...
                const listItem = $("<li>");
                listItem.append(
                    $("<span>").addClass("result").text(`Monthly (before tax): 
                    ${monthlyAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`),
                    " | ",
                    $("<span>").addClass("result").text(`Monthly tax: 
                    ${monthlyTax.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`),
                    " | ",
                    $("<span>").addClass("result").text(`Monthly (after tax): 
                    ${netMonthlyAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`),
                    "<br>",
                    $("<span>").addClass("result").text(`Yearly (before tax): 
                    ${yearlyAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`),
                    " | ",
                    $("<span>").addClass("result").text(`Yearly tax: 
                    ${yearlyTax.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`),
                    " | ",
                    $("<span>").addClass("result").text(`Yearly (after tax): 
                    ${netYearlyAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`)
                );
